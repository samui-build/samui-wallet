...
describe("db-account-set-active", () => {
  ...
  describe("expected behavior", () => {
    it("should set an account and its first wallet to active", async () => {
      ...
      // ACT
      const activeAccountIdBeforeSetActive = await dbPreferenceFindUniqueByKey(
        db,
        "activeAccountId",
      );
      const activeWalletIdBeforeSetActive = await dbPreferenceFindUniqueByKey(
        db,
        "activeWalletId",
      );

      await dbAccountSetActive(db, idAccount2);
      const activeAccountIdAfterSetActive = await dbPreferenceFindUniqueByKey(
        db,
        "activeAccountId",
      );
      const activeWalletIdAfterSetActive = await dbPreferenceFindUniqueByKey(
        db,
        "activeWalletId",
      );
  ...
  describe("unexpected behavior", () => {
    ...
    it("should throw an error when account does not exist", async () => {
      ...
      // ACT & ASSERT
      await expect(dbAccountSetActive(db, nonExistentId)).rejects.toThrow(
    ...
    it("should handle account with no wallets gracefully", async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        `There are no wallets in account ${idAccount}`,
      );
    });
  });
});

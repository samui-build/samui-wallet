...
describe("db-wallet-set-active", () => {
  ...
  describe("expected behavior", () => {
    it("should set an wallet and its related account to active", async () => {
      ...
      // ACT
      const activeAccountIdBefore = await dbPreferenceFindUniqueByKey(
        db,
        "activeAccountId",
      );
      const activeWalletIdBefore = await dbPreferenceFindUniqueByKey(
        db,
        "activeWalletId",
      );

      await dbWalletSetActive(db, idWallet2);
      const activeAccountIdAfter = await dbPreferenceFindUniqueByKey(
        db,
        "activeAccountId",
      );
      const activeWalletIdAfter = await dbPreferenceFindUniqueByKey(
        db,
        "activeWalletId",
      );
  ...
  describe("unexpected behavior", () => {
    it("should throw an error when wallet does not exist", async () => {
      ...
      // ACT & ASSERT
      await expect(dbWalletSetActive(db, nonExistentId)).rejects.toThrow(
        `Wallet with id ${nonExistentId} not found`,
      );
    });
  });
});

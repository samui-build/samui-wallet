...
describe("db-account-find-many", () => {
  ...
  describe("expected behavior", () => {
    it("should find many accounts with wallets", async () => {
      ...
      await dbWalletCreate(
        db,
        testWalletInputCreate({ accountId: account1Id }),
      );
      await dbWalletCreate(
        db,
        testWalletInputCreate({ accountId: account1Id }),
      ...
      await dbWalletCreate(
        db,
        testWalletInputCreate({ accountId: account2Id }),
      );
      await dbWalletCreate(
        db,
        testWalletInputCreate({ accountId: account3Id }),
      );
      await dbWalletCreate(
        db,
        testWalletInputCreate({ accountId: account3Id }),
      ...
      expect(
        items.map((i) => ({
          id: i.id,
          order: i.order,
          walletsLength: i.wallets.length,
        })),
      ).toEqual(
    ...
    it("should find many accounts by a partial name", async () => {
      ...
      expect(items.map((i) => i.name)).toEqual(
        expect.arrayContaining([account1.name, account2.name]),
      );
    });
  ...
  describe("unexpected behavior", () => {
    ...
    it("should throw an error when finding accounts fails", async () => {
      ...
      // ACT & ASSERT
      await expect(dbAccountFindMany(db)).rejects.toThrow(

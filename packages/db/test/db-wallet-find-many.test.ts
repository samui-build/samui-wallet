  ...
  describe("expected behavior", () => {
    it("should find many wallets for an account", async () => {
      ...
      expect(items.map((i) => i.name)).toEqual(
        expect.arrayContaining([wallet1.name, wallet2.name]),
      );
    });
    ...
    it("should find many wallets for an account by type", async () => {
      ...
      expect(items).toHaveLength(2);
      expect(items.map((i) => i.name)).toEqual(
        expect.arrayContaining([wallet1.name, wallet3.name]),
      );
    });
    ...
    it("should find many wallets for an account by a partial name and type", async () => {
      ...
      const wallet3 = testWalletInputCreate({
        accountId,
        name: "Savings",
        type: "Watched",
      });
      const wallet4 = testWalletInputCreate({
        accountId,
      ...
      expect(items.map((i) => i.name)).toEqual(
        expect.arrayContaining([wallet1.name]),
      );
    });

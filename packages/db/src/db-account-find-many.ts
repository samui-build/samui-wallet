...
export async function dbAccountFindMany(
  db: Database,
  ...
  return db.transaction("r", db.accounts, db.wallets, async () => {
    const [
      { data: dataAccounts, error: accountsError },
      { data: dataWallets, error: errorWallets },
    ] = await Promise.all([
      tryCatch(
        db.accounts
          .orderBy("order")
    ...
    return [
      ...dataAccounts.map((account) => {
        return {
          ...account,
          wallets: [
            ...dataWallets.filter((wallet) => wallet.accountId === account.id),
          ],
        };
      }),
    ];

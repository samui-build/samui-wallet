...
export async function getDbTableRecord(ctx: DbContext, table: DbTableName, id: string): Promise<DbRecord | undefined> {
  return (await ctx.db.table(table).get(id)) as DbRecord | undefined
}

export function getDbTableRecords(ctx: DbContext, table: DbTableName): Promise<DbRecord[]> {

  ...
  return (
    <ExplorerUiDetailGrid>
      <ExplorerUiDetailGrid cols={2}>
        <ExplorerUiDetailRow label={t(($) => $.accountType)} value={<Badge variant="outline">{type}</Badge>} />
        {account?.exists ? (
          <ExplorerUiDetailRow
            label={t(($) => $.owner)}
            value={
              account?.programAddress ? (
                <ExplorerUiLinkAddress address={account?.programAddress} basePath={basePath} />
              ) : null
            }
          />
        ) : null}
      </ExplorerUiDetailGrid>
      {account?.exists ? <ExplorerFeatureAccountInfoContent account={account} basePath={basePath} /> : null}
    </ExplorerUiDetailGrid>
  )
}

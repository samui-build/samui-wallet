...
export function useSplTokenCreateTokenMint(props: {
  cluster: Cluster;
  wallet: Wallet;
}) {
  ...
  return useMutation({
    mutationFn: async (
      input: Omit<SplTokenCreateTokenMintOptions, "feePayer">,
    ) => {
      if (!props.wallet.secretKey) {
        throw new Error("Missing wallet secret key");
      }
      const feePayer = await createKeyPairSignerFromJson({
        json: props.wallet.secretKey,
      });

...
export function SettingsFeatureAccountAddWallet() {
  ...
  async function createWalletDerived(account: Account) {
    try {
      await deriveWallet.mutateAsync({
        index: wallets?.length ?? 0,
        item: account,
      });
  ...
  async function createWalletImported(accountId: string) {
    ...
    try {
      const { publicKey, secretKey } =
        await importKeyPairToPublicKeySecretKey(input);
  ...
  async function createWalletWatched(accountId: string) {
    const publicKey = window.prompt(
      "What is the public key you want to watch?",
    );
      ...
      <div className="space-y-6">
        <Item variant="outline">
          ...
          <ItemContent>
            ...
            <ItemDescription>
              Derive a wallet from the Account&#39;s mnemonic
            </ItemDescription>
          </ItemContent>
          <ItemActions>
            <Button
              onClick={() => createWalletDerived(item)}
              size="sm"
              variant="outline"
            >
              Derive
        ...
        <Item variant="outline">
          ...
          <ItemContent>
            ...
            <ItemDescription>
              Import a wallet from a base85 secret key or byte array
            </ItemDescription>
          </ItemContent>
          <ItemActions>
            <Button
              onClick={() => createWalletImported(item.id)}
              size="sm"
              variant="outline"
            >
              Import
        ...
        <Item variant="outline">
          ...
          <ItemContent>
            ...
            <ItemDescription>
              Watch a wallet by public key without importing a secret
            </ItemDescription>
          </ItemContent>
          <ItemActions>
            <Button
              onClick={() => createWalletWatched(item.id)}
              size="sm"
              variant="outline"
            >
              Watch

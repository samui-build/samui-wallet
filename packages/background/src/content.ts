...
export function handlers() {
  ...
  onMessage(
    ...
    async ({ data }) => await sendMessage("signAndSendTransaction", data),
  );
  onMessage("signIn", async ({ data }) => await sendMessage("signIn", data));
  onMessage(
    "signMessage",
    async ({ data }) => await sendMessage("signMessage", data),
  );
  onMessage(
    "signTransaction",
    async ({ data }) => await sendMessage("signTransaction", data),
  );
}

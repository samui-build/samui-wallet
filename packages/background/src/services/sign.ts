...
export const [registerSignService, getSignService] = defineProxyService(
  "SignService",
  () => ({
    signAndSendTransaction: async (
      ...
      for (const input of inputs) {
        const decoded = getTransactionDecoder().decode(
          ensureUint8Array(input.transaction),
        );
        ...
        results.push({
          signature: new Uint8Array(
            getBase58Encoder().encode(getSignatureFromTransaction(transaction)),
          ),
        });
      }

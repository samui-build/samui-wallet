...
type DataType<T extends Requests["type"]> = Extract<
  Requests,
  { type: T }
>["data"];
...
type ResolveType<T extends Requests["type"]> = Extract<
  Requests,
  { type: T }
> extends { resolve: (data: infer R) => void }
  ? R
  : never;
...
class RequestService {
  ...
  async create<T extends Requests["type"]>(
    type: T,
    data: DataType<T>,
  ): Promise<ResolveType<T>> {
    if (this.request) {

...
describe("derive-from-mnemonic-at-index", () => {
  describe("expected behavior", () => {
    it("should derive a wallet at a specific index from a mnemonic", async () => {
      ...
      // ACT
      const result = await deriveFromMnemonicAtIndex({
        derivationIndex,
        mnemonic,
      });
  ...
  describe("unexpected behavior", () => {
    it("should throw an error for an invalid mnemonic", async () => {
      ...
      // ACT & ASSERT
      await expect(deriveFromMnemonicAtIndex({ mnemonic })).rejects.toThrow(
        "Invalid mnemonic",
      );
    });
    ...
    it("should throw an error for a mnemonic with a word not in the wordlist", async () => {
      ...
      expect.assertions(1);
      const mnemonic =
        "afford canoe observe bone master venture shoot erode regular coffee dose blockchain";
    ...
    it("should throw an error for a negative derivation index", async () => {
      ...
      // ACT & ASSERT
      await expect(
        deriveFromMnemonicAtIndex({ derivationIndex, mnemonic }),
      ).rejects.toThrow("Error creating KeyPair signer from mnemonic");
    });
  });
});

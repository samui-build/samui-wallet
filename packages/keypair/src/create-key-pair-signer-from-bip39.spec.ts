...
describe("create-key-pair-signer-from-bip39", () => {
  describe("expected behavior", () => {
    it("should create a keypair signer from a mnemonic", async () => {
      ...
      expect.assertions(1);
      const mnemonic =
        "afford canoe observe bone master venture shoot erode regular coffee dose cute";
    ...
    it("should create a keypair signer from a mnemonic with passphrase", async () => {
      ...
      expect.assertions(1);
      const mnemonic =
        "tower mandate pupil dance furnace scheme wisdom envelope next fantasy flavor merit";
      const address = "SFDXaLas2FdpiGn9gtvaxx5jFG7L7QAwY8KvWCu6B5d";
      const passphrase = "foobar";
      // ACT
      const result = await createKeyPairSignerFromBip39({
        mnemonic,
        passphrase,
      });
      // ASSERT
  ...
  describe("unexpected behavior", () => {
    it("should throw an error for an invalid mnemonic", async () => {
      ...
      // ACT & ASSERT
      await expect(createKeyPairSignerFromBip39({ mnemonic })).rejects.toThrow(
        "Invalid mnemonic",
      );
    });
    ...
    it("should throw an error for a mnemonic with a word not in the wordlist", async () => {
      ...
      expect.assertions(1);
      const mnemonic =
        "dummy canoe observe bone master venture shoot erode regular coffee dose cute";

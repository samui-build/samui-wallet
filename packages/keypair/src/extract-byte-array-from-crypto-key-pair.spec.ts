...
describe("extract-byte-array-from-crypto-key-pair", () => {
  describe("expected behavior", () => {
    it("should extract a 64-byte array representing the private and public keys", async () => {
      ...
      const keyPairSigner = await createKeyPairSignerFromBip39({
        extractable: true,
        mnemonic,
      });
  ...
  describe("unexpected behavior", () => {
    it("should throw an error if the key is not extractable", async () => {
      ...
      // ACT & ASSERT
      await expect(
        extractByteArrayFromCryptoKeyPair(cryptoKeyPair),
    ...
    it("should throw an error for null input", async () => {
      ...
      await expect(
        extractByteArrayFromCryptoKeyPair(invalidInput),
      ).rejects.toThrow(`Cannot read properties of null (reading 'publicKey')`);
    });

...
describe("create-key-pair-signer-from-bip44", () => {
  describe("expected behavior", () => {
    it("should create a keypair signer from a mnemonic set", async () => {
      ...
      expect.assertions(2);
      const mnemonic =
        "kid search occur scrub tumble dove transfer prevent mirror brush gravity fork";
      const addresses = [
    ...
    it("should create a keypair signer from a mnemonic set with passphrase", async () => {
      ...
      expect.assertions(2);
      const mnemonic =
        "toy inner shy cup come mansion nurse curtain anger spatial blush vessel";
      const addresses = [
      ...
      // ACT
      const result = await createKeyPairSignerFromBip44({
        mnemonic,
        passphrase,
        to: 4,
      });
  ...
  describe("unexpected behavior", () => {
    it("should throw an error for an invalid mnemonic", async () => {
      ...
      // ACT & ASSERT
      await expect(
        createKeyPairSignerFromBip44({ mnemonic, to: 1 }),
    ...
    it("should throw an error for a mnemonic with a word not in the wordlist", async () => {
      ...
      expect.assertions(1);
      const mnemonic =
        "dummy canoe observe bone master venture shoot erode regular coffee dose zebra";
    ...
    it("should throw an error if to is 0", async () => {
      ...
      // ACT & ASSERT
      await expect(
        createKeyPairSignerFromBip44({ mnemonic, to: 0 }),
      ).rejects.toThrow("to must be a positive number");
    });
    ...
    it("should throw an error if from is negative", async () => {
      ...
      // ACT & ASSERT
      await expect(
        createKeyPairSignerFromBip44({ from: -1, mnemonic }),
      ).rejects.toThrow("from must be a positive number");
    });
    ...
    it("should throw an error if to is not greater than from", async () => {
      ...
      // ACT & ASSERT
      await expect(
        createKeyPairSignerFromBip44({ from: 1, mnemonic, to: 1 }),
      ).rejects.toThrow("to must be greater than from");
    });
  });
});

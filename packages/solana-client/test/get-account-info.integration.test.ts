...
describe('get-account-info', async () => {
  ...
  describe('expected behavior', () => {
    ...
    it('should get account info for ata and mint', async () => {
      ...
      // ASSERT
      // @ts-expect-error these values are here but the type does not agree
      expect(resultAta.value?.data.program).toBe('spl-token')
      // @ts-expect-error these values are here but the type does not agree
      expect(resultAta.value?.data.parsed.type).toBe('account')
      // @ts-expect-error these values are here but the type does not agree
      expect(resultMint.value?.data.program).toBe('spl-token')
      // @ts-expect-error these values are here but the type does not agree
      expect(resultMint.value?.data.parsed.type).toBe('mint')
    })
  })
})

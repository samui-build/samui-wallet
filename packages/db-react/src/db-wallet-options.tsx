...
export type DbWalletCreateMutateOptions = MutateOptions<
  string,
  Error,
  { input: WalletInputCreate }
>;
export type DbWalletDeleteMutateOptions = MutateOptions<
  void,
  Error,
  { id: string }
>;
export type DbWalletSetActiveMutateOptions = MutateOptions<
  void,
  Error,
  { id: string }
>;
export type DbWalletUpdateMutateOptions = MutateOptions<
  number,
  Error,
...
export const dbWalletOptions = {
  create: (props: DbWalletCreateMutateOptions = {}) =>
    mutationOptions({
      mutationFn: ({ input }: { input: WalletInputCreate }) =>
        dbWalletCreate(db, input),
      ...props,
    }),
  ...
  update: (props: DbWalletUpdateMutateOptions = {}) =>
    mutationOptions({
      mutationFn: ({ id, input }: { id: string; input: WalletInputUpdate }) =>
        dbWalletUpdate(db, id, input),
      ...props,
    }),
};

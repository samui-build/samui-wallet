...
export type DbClusterCreateMutateOptions = MutateOptions<
  string,
  Error,
  { input: ClusterInputCreate }
>;
export type DbClusterDeleteMutateOptions = MutateOptions<
  void,
  Error,
  { id: string }
>;
export type DbClusterUpdateMutateOptions = MutateOptions<
  number,
  Error,
  { input: ClusterInputUpdate }
>;

export const dbClusterOptions = {
  create: (props: DbClusterCreateMutateOptions = {}) =>
    mutationOptions({
      mutationFn: ({ input }: { input: ClusterInputCreate }) =>
        dbClusterCreate(db, input),
  ...
  update: (props: DbClusterUpdateMutateOptions) =>
    mutationOptions({
      mutationFn: ({ id, input }: { id: string; input: ClusterInputUpdate }) =>
        dbClusterUpdate(db, id, input),
